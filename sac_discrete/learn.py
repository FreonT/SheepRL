...
def calc_policy_loss(model, states, weights, alpha):
    ...
    entropies = -torch.sum(
        action_probs * log_action_probs, dim=2, keepdim=True)
    # expectations of Q
    q = torch.sum(torch.min(q1, q2) * action_probs, dim=2, keepdim=True)
...
def calc_current_q(model, states, actions):
    ...
    curr_q1 = curr_q1.gather(2, actions.unsqueeze(2).long())
    curr_q2 = curr_q2.gather(2, actions.unsqueeze(2).long())
...
def calc_target_q(model, rewards, next_states, dones, alpha, gamma_n):
    with torch.no_grad():
        ...
        next_q1, next_q2 = model.critic(next_states)
        next_q = (action_probs * (
